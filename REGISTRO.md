...
- Segunda pasada del marco v2 con Daniel (call 30 jul, `sprint 15/resumen_call.md`; transcripción en `sprint 15/DANIEL 30 DE JULIO.transcripcion.txt`, gitignoreada). Decisión de fondo: **desacoplar el marco de Scrum** y contarlo de lo general a lo particular. Daniel bajó por escrito la narrativa de la **v3**, volcada como tabla de pasos en el resumen: **Sommerville** (proceso de desarrollo) → **V-model** (los 2 puntos de feedback: requisitos y aceptación) → **Scrum** solo como ejemplo (sin IA) → "Feedback con IA" (categorías de artículos con nombre, agnósticas al modelo de proceso) → gráfico de escenarios → aspectos no considerados / problemas abiertos. Sigue siendo estado del arte, sin propuesta propia todavía.
- Resultado: `sprint 16/MARCO_PROCESO_FEEDBACK_v3.md` completo (secciones 1–6, según la narrativa de Daniel). Lo central: sec. 4 reorganizada por "qué operación hace la IA sobre el ciclo", con todas las asignaciones **verificadas contra los PDF** (4 de la v2 resultaron falsas); la revisión complementaria con vocabulario de aceptación confirmó el hueco: en lo relevado no hay estudios de la interacción de validación del stakeholder sobre software construido (las etapas 5 y 6 del proceso de aceptación de Sommerville quedan sin trabajos). Figuras **literales** de Sommerville (Figs. 2.7 y 8.11) extraídas del ejemplar (la imagen web del V-model resultó ser stock de Shutterstock, descartada). `lindenberg2025business` resuelto por **lectura de abstract**. Fichas de Virk, Fawzy y Sharma verificadas contra PDF (autores corregidos; Sharma es CHI 2026) y sumadas a `REFERENCIAS.xlsx`.
- Reestructura final a **resumen ejecutivo** (10-ago, feedback de Victoria): evidencia de mercado y categorías 4.1–4.6 como tablas; la tabla A2a–A5 reemplazada por párrafos más el **BPMN del v2 embebido** en 3.2; tabla S1–S4 eliminada (el vínculo categorías-escenarios va en la figura, regenerada sin restos del v2); 4.7 y 4.8 comprimidos y sin afirmaciones de saturación (todo escopado a "lo relevado"); strings de búsqueda movidos a `NOTAS_MEMORIA.md`. De ~530 a ~280 líneas.
- **Pendiente:** enviar la v3 a Daniel. Para la reunión, 3 decisiones: (a) tramo técnico 4.6, (b) si el BPMN del v2 se queda en 3.2, (c) "firewall" de feedback como línea de la PoC.
