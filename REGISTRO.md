...
- Resultado: `sprint 16/MARCO_PROCESO_FEEDBACK_v3.md` completo (secciones 1–6, según la narrativa de Daniel). Lo central: sec. 4 reorganizada por "qué operación hace la IA sobre el ciclo", con todas las asignaciones **verificadas contra los PDF** (4 de la v2 resultaron falsas); **búsqueda dirigida** sobre aceptación con resultado 24 trabajos tipo A (generan el artefacto) y **0 tipo B** (interacción de validación del stakeholder): el hueco es real y es el objeto de la tesis, legible también sobre las 6 etapas de aceptación de Sommerville (etapas 5 y 6 sin trabajos). Figuras **literales** de Sommerville (Figs. 2.7 y 8.11) extraídas del ejemplar (la imagen web del V-model resultó ser stock de Shutterstock, descartada). `lindenberg2025business` resuelto por **lectura de abstract** (descubre procesos, no valida software; sin humanos en la evaluación). Fichas de Virk, Fawzy y Sharma verificadas contra PDF (autores corregidos; Sharma es CHI 2026). Notas de trabajo movidas a `NOTAS_MEMORIA.md` y `BUSQUEDAS_ORT.md`; barrida de estilo (sin em dashes).
- **Pendiente:** enviar la v3 a Daniel. Para la reunión, 3 decisiones: (a) tramo técnico 4.6, (b) tabla A2a–A5 de la sec. 3, (c) "firewall" de feedback como línea de la PoC.
