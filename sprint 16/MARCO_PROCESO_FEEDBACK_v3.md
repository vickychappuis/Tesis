...
- **C.** Examinar la transformación del rol del cliente, del analista funcional y del equipo técnico.
- **D.** Analizar el impacto en la validación temprana de reglas de negocio, flujos y criterios de aceptación.
- **E.** Describir efectos percibidos en tiempos de ajuste, claridad de requerimientos, alineación y coordinación.
...
|---|---|---|---|---|---|
| **4.1** | **Emite el feedback**: impersonación de stakeholders (objetivo **C**) | [gris: Pirozzi 2024, `damian2025reconnect`] + [producto] | hay "stakeholder" disponible en cualquier momento del desarrollo, sin coordinar agendas; el stakeholder real deja de ser cuello de botella y pasa de emisor a **validador** de lo que la IA respondió | no es el stakeholder real: el feedback puede sonar plausible y estar mal; REConnect pide que el humano quede como *"curator of AI outputs"* (límite a la **sustitución**, no a la técnica) | ⚠ **sin respaldo del corpus**: se declara [gris]+[producto] |
| **4.2** | **Captura lo que se dijo**: asistentes de reunión (objetivos **B** y **E**) | [corpus: `cabrero2024exploring`] | lo hablado queda como registro accionable (resumen, tickets) en vez de perderse; en el estudio, además, detectó riesgos de sobrecompromiso e impedimentos que el equipo no veía | qué se pierde en el resumen; si la presencia del asistente cambia lo que la gente dice | cubre solo **reuniones internas** (`Sprint Review` = 0, `client` = 0); **no transcribe** (planillas cargadas a mano): entra como contexto, no como núcleo |
| **4.3** | **Trabaja el artefacto de requisito**: reformular el enunciado, juzgar su calidad, rutear input disperso (objetivos **B** y **E**) | [corpus: `mircea2026supporting`, `abbasi2025towards`] · [gris: `geyer2025epics`, `torun2025bugtracking`] · [producto: `kraftful`, `versive`] | el enunciado del stakeholder llega al equipo más claro y completo sin dejar de ser suyo. Mircea lo mide (26 participantes, 130 pares): todas las dimensiones mejoran (p&lt;.001), el 43 % de las revisiones destapa aspectos que el autor no había dicho y consideraba importantes, solo el 5 % introduce errores; el LLM reformula y el humano aprueba (*"AI-in-the-Loop"*) | que el LLM *sustituya* la intención original en vez de articularla; que el criterio del evaluador automático reemplace al del negocio sin que nadie lo note | Mircea: participantes técnicos, **sin actores de negocio** ni software construido; Abbasi: **sin evaluación empírica** y el usuario es el ingeniero, no el stakeholder; Geyer: evaluado por 17 PMs, no clientes; Torun: el único del corpus sobre software ya construido |
| **4.4** | **Construye algo mirable o ejecutable**: materialización temprana. (a) la opera el equipo; (b) el usuario de negocio, sin intermediario técnico (objetivos **B**, **C** y **D**) | (a) [corpus: `kretzer2025closing`] · [gris: `busany2024bi`, `alabsi2026empirical`]; (b) [corpus: `robinson2025requirements`] · [producto: `emergent`, `pmagent`] · [gris: prensa profesional] | adelanta el momento en que el stakeholder puede reaccionar: opina sobre algo concreto y usable, no sobre una descripción, y eso destapa requisitos que no sabía que tenía (*"seeing a tangible product can unearth elements of the requirements that were assumed subconsciously but not articulated"*, Robinson) | el prototipo puede inducir la respuesta en vez de revelarla; validar rápido sacrifica UX **[general: Cornide-Reyes 2025]**; el pivote de Tusk y Sweep sugiere que el modo autónomo no cierra | Kretzer: intra-equipo (`client` = 0); Alabsi: **sin participantes** (stakeholder simulado, una sola pasada; su *"enables earlier validation"* es inferencial); Robinson: **visión sin empiria** |
| **4.5** | **Valida contra reglas y procesos de negocio**: la mitad del planteo de esta tesis (objetivo **D**) | [corpus: `lindenberg2025business`, solo abstract] | previsto, sin evidencia todavía: validar temprano y de forma continua que lo construido respeta las reglas y flujos del negocio, sin esperar a la instancia de aceptación | previsto: que la regla capturada esté mal y todo se valide contra esa versión sin que el negocio lo note; el criterio del negocio queda codificado y deja de discutirse con la persona | **sin trabajos de validación propiamente dicha**: Lindenberg *descubre* procesos (aguas arriba, sin humanos en la evaluación), no valida software construido contra reglas; el ancla conceptual es Dumas secs. 4.4 y 5.4.2 (la A3a de la sec. 3) |
| **4.6** | **Tramo técnico** (anecdótico): generación de código con validación humana | [producto: `devin`, `codegen`, `tusk`, `sweep`] | | | **fuera del alcance de la tesis**: no se juzga la generación de código; se deja registrada la evidencia de mercado solo como contexto |
